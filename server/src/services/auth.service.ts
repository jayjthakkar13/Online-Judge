...
export default class AuthService {
	...
	private static validateCredentials(email: string, password: string, authType: AuthTypes, name?: string): ValidationResult {
		const errors: string[] = [];
      ...
      if (name!.trim().length === 0) errors.push("Name is a required field.");
	...
	public static async register(name: string, email: string, password: string): Promise<ResponseData> {
		const { isValid, errors } = this.validateCredentials(email, password, AuthTypes.Register, name);
